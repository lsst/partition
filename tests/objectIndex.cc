...
BOOST_AUTO_TEST_CASE(ObjectIndexTest) {
    ...
    BOOST_REQUIRE_NO_THROW({
        ObjectIndex::instance().isOpen();
    });
    BOOST_CHECK_EQUAL(ObjectIndex::instance().isOpen(), false);
    BOOST_REQUIRE_NO_THROW({
        ObjectIndex::instance().mode();
    });
    ...
    BOOST_CHECK_THROW({
        ObjectIndex::instance().open("", dialect);
    }, std::invalid_argument);
    BOOST_CHECK_THROW({
        ObjectIndex::instance().open("file:///", dialect);
    }, std::invalid_argument);
    ...
    BOOST_REQUIRE_NO_THROW({
        chunkSubChunk = ObjectIndex::instance().read(validId);
    });
    ...
    BOOST_CHECK_THROW({
        chunkSubChunk = ObjectIndex::instance().read(notValidId);
    }, std::out_of_range);
    BOOST_REQUIRE_NO_THROW({
        ObjectIndex::instance().close();
    });
    ...
    BOOST_REQUIRE_NO_THROW({
        chunkSubChunk = ObjectIndex::instance().read(anotherValidId);
    });
